...
- [ ] Set the correct kinematic viscosity ν in `constant/
      physicalProperties` (matching the Re_τ=180 / air properties used
      to derive the mesh resolution — ν=1.4604e-05 m²/s).
- [ ] Set correct boundary conditions in `0/U` and `0/p` for all
      patches (`wall`, `left`, `right`, `inner`, `outer`) — periodic
      streamwise (`left`/`right`) and spanwise (`inner`/`outer`)
      cyclic BCs plus no-slip walls, driven by either a fixed pressure
      gradient or a momentum source (needs deciding).
- [ ] Check discretization schemes in `system/fvSchemes` (currently
      whatever `foamCloneCase`/tutorial defaults were copied in — needs
      review for DNS suitability, not just RAS-tutorial defaults).
- [ ] Calculate deltaT from a target Courant number, using the mesh's
      own Δx/Δy/Δz and an estimated velocity scale (u_τ or U_c).
- [ ] Set up parallel decomposition (`decomposeParDict`) — ~3.66M hex
      cells is not practical to run serially.
- [ ] Run the simulation for a short time as a smoke test to confirm it
      actually runs before committing to a full run; decide the real
      end time (needs to be long enough to reach a statistically
      stationary state before time-averaging can start — TBD how many
      flow-through times or eddy turnover times that requires).
...
- [ ] Start the FEM build order from `../overview.md` (steady 2D Stokes +
      ...
      profile) — not yet started. (Open since the project began; not a
      recent addition — kept last deliberately.)

## Learnings

<!-- Things discovered along the way: gotchas, bugs found and their root
     cause, results that changed the plan. Dated, newest first. Each entry:
     what you found, then why it matters -- the "why" is what makes an old
     entry still useful later. -->
...
- **2026-09-03** — Final grid: 192×129×160 points (191×128×159
  ...
  0.011566 (retuned from the 120-pt value below — the Bump coefficient
...
- **2026-09-03** — Retuned `Transfinite Curve{12,14}` (wall-normal) from
  ...
  two entries above (the point-vs-division off-by-one, and the final
  192×129×160 grid); superseded, don't use the N=120/Bump 0.01054
  numbers here as current.
...
- **2026-09-02** — DNS near-wall resolution target: first-cell y⁺ ≲ 1,
  ...
  into an actual mesh spacing once Re_τ is picked.
